...
async function cancelOrder(event) {
  ...
  if (!window.confirm(`Hủy đơn ${button.dataset.orderNumber}? Chỉ đơn chưa thanh toán và chưa giao nhận mới hủy được.`)) return;
  button.disabled = true;
  const { error } = await db.rpc("cancel_my_order", { p_order_id: button.dataset.orderCancel, p_reason: cancellationReason("customer") });
