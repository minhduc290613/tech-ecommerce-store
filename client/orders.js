...
function renderOrder(order) {
  ...
  const paymentAction = order.status === "pending_payment" ? `<button class="payment-confirm-button" data-payment-confirm="${escapeHtml(order.id)}" data-order-number="${escapeHtml(order.order_number)}" data-total="${Number(order.total_amount)}"><i class="fa-brands fa-zalo"></i> Đã thanh toán — liên hệ Zalo</button>` : `<span class="payment-complete"><i class="fa-solid fa-circle-check"></i> ${escapeHtml(paymentCopy[order.status] || order.status)}</span>`;
