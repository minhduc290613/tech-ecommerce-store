...
function renderCurrencySettings() { const select = $("#mantisDefaultCurrency"); const rateInput = $("#mantisCurrencyRate"); const rows = $("#mantisCurrencyRows"); if (!select || !rateInput || !rows) return; const codes = Object.keys(CURRENCY_RATES); select.innerHTML = codes.map(code => `<option value="${code}">${code}</option>`).join(""); select.value = state.currency; rateInput.value = String(CURRENCY_RATES[state.currency] || 1); rows.innerHTML = codes.map(code => `<tr><td class="fw-semibold">${code}</td><td>${CURRENCY_RATES[code]}</td><td><span class="badge text-bg-success-subtle text-success-emphasis">${t("enabled")}</span></td></tr>`).join(""); }
function handleCurrencySave(event) { event.preventDefault(); const select = $("#mantisDefaultCurrency"); const rateInput = $("#mantisCurrencyRate"); const rate = Number(rateInput.value); if (!select.value || !Number.isFinite(rate) || rate <= 0) { showToast(state.language === "vi" ? "Hãy nhập tỷ giá lớn hơn 0." : "Enter an exchange rate greater than 0.", "error"); return; } state.currency = select.value; CURRENCY_RATES[state.currency] = rate; localStorage.setItem("nexora-currency", state.currency); localStorage.setItem(`nexora-rate-${state.currency}`, String(rate)); renderCurrencySettings(); renderCharts(state.orders); renderLatestOrders(state.orders); if (state.orders.length) loadMetrics(); showToast(state.language === "vi" ? "Đã lưu cài đặt tiền tệ hiển thị." : "Display currency settings saved.", "success"); }
