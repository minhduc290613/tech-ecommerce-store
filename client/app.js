...
function updatePaymentQR() {
  ...
  if (state.activePaymentMethod === "vietqr") {
    ...
    const qrUrl = getTrustedZaloPayQrUrl(state.settings?.payment_zalopay_qr_url);
    if (!qrUrl) { showQRState("Shop chưa cấu hình ảnh QR ZaloPay HTTPS. Vui lòng chọn phương thức khác hoặc liên hệ shop."); return; }
    els.qrImage.hidden = false;
