...
function applySettings() { const s = state.settings; document.title = `${s.site_name} Tech Store | ${s.hero_title}`; $("#announcementText").textContent = s.announcement_text; $("#heroKicker").textContent = s.hero_kicker; $("#heroTitlePlain").textContent = s.hero_title; $("#heroTitleEmphasis").textContent = s.hero_emphasis; $("#heroDescription").textContent = s.hero_description; $("#footerTagline").innerHTML = escapeHtml(s.site_tagline).replace(/\n/g, "<br />"); $("#footerSupportEmail").textContent = s.support_email; $("#footerSupportEmail").href = `mailto:${s.support_email}`; $("#footerSupportHours").textContent = s.support_hours; $("#footerAddress").textContent = s.address_text; if (s.hero_image_url) $("#heroImage").src = s.hero_image_url; $$("[data-site-logo]").forEach((img) => { if (s.logo_url) img.src = s.logo_url; }); $$("[data-site-name]").forEach((item) => item.textContent = s.site_name); renderFooterContacts(s); renderZaloConfirmation(); }
  ...
  return `
    <article class="product-card ${purchasable ? "" : "out-of-stock"}">
      ...
      <div class="product-content">
        ...
        <div class="price-row"><strong class="sale-price">${formatCurrency(product.price)}</strong>${oldPrice}</div>${renderProductSpecChips(product)}
...
function renderProductSpecChips(product) { const specs = product.technical_specs || {}; const priority = ["processor", "chipset", "ram", "storage"]; const items = priority.filter((key) => specs[key]).slice(0, 2); return items.length ? `<div class="product-spec-chips">${items.map((key) => `<span class="product-spec-chip">${PRODUCT_SPEC_LABELS[key]}: ${escapeHtml(specs[key])}</span>`).join("")}</div>` : ""; }
function renderQuickViewSpecs(product) { const specs = product.technical_specs || {}; const entries = Object.entries(specs).filter(([, value]) => value); let panel = $("#quickViewSpecs"); if (!panel) { panel = document.createElement("dl"); panel.id = "quickViewSpecs"; panel.className = "technical-specs"; els.quickViewPrice.after(panel); } panel.hidden = !entries.length; panel.innerHTML = entries.map(([key, value]) => `<div class="technical-spec"><dt>${escapeHtml(PRODUCT_SPEC_LABELS[key] || key)}</dt><dd>${escapeHtml(value)}</dd></div>`).join(""); }
