    ...
    <section class="modal account-center-modal" id="accountCenterModal" role="dialog" aria-modal="true" aria-labelledby="accountCenterTitle" hidden>
      <div class="modal-card account-center-card">
        ...
        <section class="account-tab" data-account-panel="delivery">
          <form id="accountDeliveryAddressForm" class="account-form compact"><div class="account-form-head"><h3>Địa chỉ nhận hàng</h3><p>Địa chỉ mặc định được tự điền khi tạo đơn. Bạn vẫn có thể điều chỉnh theo từng đơn trong giỏ hàng.</p></div><label>Địa chỉ nhận hàng mặc định<textarea id="accountDeliveryAddress" maxlength="500" autocomplete="street-address" required placeholder="Số nhà, đường, phường/xã, quận/huyện, tỉnh/thành"></textarea></label><button class="button button-primary" id="saveDeliveryAddress" type="submit">Lưu địa chỉ <i class="fa-solid fa-location-dot"></i></button></form>
        </section>
...
function currentDeliveryProfile() { return { phone: state.profile?.delivery_phone || "", address: state.profile?.default_shipping_address || "" }; }
async function saveAccountDetails({ displayName, username, phone, address, button, loadingLabel, successMessage }) { const delivery = normalizeDeliveryProfile({ phone, address }); if (!delivery.valid) return notify(delivery.field === "phone" ? "Hãy cập nhật số điện thoại trong mục Bảo mật trước." : "Hãy cập nhật địa chỉ trong mục Địa chỉ nhận hàng trước.", "error"); setLoading(button, true, loadingLabel); const { error } = await db.rpc("update_my_account", { p_display_name: displayName, p_username: username, p_delivery_phone: delivery.phone, p_default_shipping_address: delivery.address }); setLoading(button, false); if (error) return notify(error.message, "error"); notify(successMessage, "success"); loadAccount(); }
async function saveProfile(event) { event.preventDefault(); const delivery = currentDeliveryProfile(); return saveAccountDetails({ displayName: $("#accountDisplayName").value, username: $("#accountUsername").value, phone: delivery.phone, address: delivery.address, button: $("#saveAccountProfile"), loadingLabel: "Đang lưu", successMessage: "Đã cập nhật hồ sơ hiển thị." }); }
async function saveDeliveryAddress(event) { event.preventDefault(); const delivery = currentDeliveryProfile(); return saveAccountDetails({ displayName: state.profile?.display_name || "", username: state.profile?.username || "", phone: delivery.phone, address: $("#accountDeliveryAddress").value, button: $("#saveDeliveryAddress"), loadingLabel: "Đang lưu", successMessage: "Đã cập nhật địa chỉ nhận hàng." }); }
async function saveDeliveryPhone(event) { event.preventDefault(); const delivery = currentDeliveryProfile(); return saveAccountDetails({ displayName: state.profile?.display_name || "", username: state.profile?.username || "", phone: $("#accountDeliveryPhone").value, address: delivery.address, button: $("#saveAccountPhone"), loadingLabel: "Đang lưu", successMessage: "Đã cập nhật số điện thoại." }); }
