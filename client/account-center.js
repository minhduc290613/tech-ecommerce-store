...
async function saveDeliveryAddress(event) { event.preventDefault(); const address = normalizeShippingAddress($("#accountDeliveryAddress").value); if (!address.valid) return notify(address.message, "error"); return saveAccountDetails({ displayName: state.profile?.display_name || "", username: state.profile?.username || "", address: address.address, button: $("#saveDeliveryAddress"), loadingLabel: "Đang lưu", successMessage: "Đã cập nhật địa chỉ nhận hàng." }); }
