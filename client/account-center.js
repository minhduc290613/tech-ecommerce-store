...
function mountAccountCenter() {
    ...
    <section class="modal account-center-modal" id="accountCenterModal" role="dialog" aria-modal="true" aria-labelledby="accountCenterTitle" hidden>
      <div class="modal-card account-center-card">
        ...
        <section class="account-tab" data-account-panel="delivery">
          ...
          <form id="accountDeliveryAddressForm" class="account-form compact"><input id="shippingAddressId" type="hidden" /><div class="account-form-head"><h3 id="shippingAddressFormTitle">Thêm địa chỉ mới</h3><p>Chỉ bạn và người có quyền xử lý giao nhận phù hợp mới dùng thông tin này cho đơn hàng.</p></div><label>Nhãn địa chỉ<input id="shippingAddressLabel" maxlength="60" placeholder="Ví dụ: Nhà riêng, Văn phòng" /></label><label>Địa chỉ nhận hàng<textarea id="accountDeliveryAddress" maxlength="500" autocomplete="street-address" required placeholder="Số nhà, đường, phường/xã, quận/huyện, tỉnh/thành"></textarea></label><label class="account-default-toggle"><input id="shippingAddressIsDefault" type="checkbox" /> Đặt làm địa chỉ mặc định</label><div class="account-inline-actions"><button class="button button-primary" id="saveDeliveryAddress" type="submit">Lưu địa chỉ <i class="fa-solid fa-location-dot"></i></button><button class="account-cancel-edit" id="cancelShippingAddressEdit" type="button" hidden>Hủy sửa</button></div></form>
        </section>
        ...
        <section class="account-tab" data-account-panel="security">
          <form id="accountPhoneForm" class="account-form compact"><div class="account-form-head"><h3>Số điện thoại</h3><p>Dùng để hỗ trợ bảo mật tài khoản và liên hệ giao nhận khi cần thiết.</p></div><label>Số điện thoại nhận hàng<input id="accountDeliveryPhone" type="tel" inputmode="tel" maxlength="20" autocomplete="tel" required placeholder="Ví dụ: 0901 234 567" /></label><small class="account-data-status is-missing" id="accountPhoneStatus"><i class="fa-solid fa-circle-info"></i> Đang đồng bộ số điện thoại…</small><button class="button button-quiet" id="saveAccountPhone" type="submit">Cập nhật số điện thoại</button></form>
  ...
  $("#accountDeliveryAddressForm").addEventListener("submit", saveDeliveryAddress);
  $("#accountShippingAddressList").addEventListener("click", handleShippingAddressAction);
  $("#cancelShippingAddressEdit").addEventListener("click", resetShippingAddressForm);
  $("#accountPhoneForm").addEventListener("submit", saveDeliveryPhone);
...
function renderAccount() {
  ...
  $("#accountDisplayName").value = profile.display_name || ""; $("#accountUsername").value = profile.username || ""; $("#accountDeliveryPhone").value = profile.delivery_phone || ""; $("#accountCurrentEmail").textContent = state.user?.email || "—";
...
async function saveAccountDetails({ displayName, username, phone = null, address = null, button, loadingLabel, successMessage }) { setLoading(button, true, loadingLabel); const { error } = await db.rpc("update_my_account", { p_display_name: displayName, p_username: username, p_delivery_phone: phone, p_default_shipping_address: address }); setLoading(button, false); if (error) return notify(error.message, "error"); notify(successMessage, "success"); loadAccount(); }
async function saveProfile(event) { event.preventDefault(); return saveAccountDetails({ displayName: $("#accountDisplayName").value, username: $("#accountUsername").value, button: $("#saveAccountProfile"), loadingLabel: "Đang lưu", successMessage: "Đã cập nhật hồ sơ hiển thị." }); }
async function saveDeliveryAddress(event) { event.preventDefault(); const address = normalizeShippingAddress($("#accountDeliveryAddress").value); if (!address.valid) return notify(address.message, "error"); const button = $("#saveDeliveryAddress"); setLoading(button, true, "Đang lưu"); const { error } = await db.rpc("save_my_shipping_address", { p_id: $("#shippingAddressId").value || null, p_label: $("#shippingAddressLabel").value, p_address: address.address, p_make_default: $("#shippingAddressIsDefault").checked }); setLoading(button, false); if (error) return notify(error.message, "error"); notify("Đã lưu địa chỉ nhận hàng.", "success"); resetShippingAddressForm(); return loadAccount(); }
async function saveDeliveryPhone(event) { event.preventDefault(); const phone = normalizeDeliveryPhone($("#accountDeliveryPhone").value); if (!phone.valid) return notify(phone.message, "error"); return saveAccountDetails({ displayName: state.profile?.display_name || "", username: state.profile?.username || "", phone: phone.phone, button: $("#saveAccountPhone"), loadingLabel: "Đang lưu", successMessage: "Đã cập nhật số điện thoại." }); }
