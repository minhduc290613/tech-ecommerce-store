...
| 2 | Chạy `supabase-schema.sql` trong Supabase SQL Editor nếu chưa chạy. |
| 3 | Chạy toàn bộ `supabase-admin.sql` trong Supabase SQL Editor để tạo bảng `admin_users`, hàm kiểm tra quyền và các chính sách RLS. |
| 4 | Tạo một tài khoản bằng email/mật khẩu mạnh ở storefront thông qua nút **Đăng nhập → Đăng ký**. |
| 5 | Thay email trong câu lệnh mẫu cuối `supabase-admin.sql`, chạy riêng câu lệnh đó để cấp quyền admin cho đúng tài khoản. |
